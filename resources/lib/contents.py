...
class Contents:
    ...
    def __init__(self, key=''):
        ...
        if Const.GET('rss') == 'true' and re.search(r'^https?://', Const.GET('rss_url')):
            ...
            # キー文字列のハッシュ
            self.hexdigest = md5(key).hexdigest() if key else ''
            # RSSのファイル名
    ...
    def contents(self):
        ...
        # keyが指定されていたらフィルタ
        if self.key: contents = filter(lambda p: p['key']==self.key, contents)
        # 開始時刻の逆順でソート
    ...
    def delete(self, gtvid=None):
        # 削除対象のgtvidをリストアップ
        todelete = [gtvid] if gtvid else map(lambda p: p['gtvid'], self.contents())
        # 削除対象がなければ終了
        ...
        if self.key: self.createrss()
        # 全コンテンツのrssファイル生成
        Contents().createrss()

    def createrss(self):
        # RSS設定がない場合は何もしないで終了
        if not self.rssroot: return
        # アイテム数
        if self.key:
            limit = None
        else:
