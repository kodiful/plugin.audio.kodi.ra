...
class Radiru(Params, Jcba):
    ...
    def setup(self, renew=False):
        ...
        if data:
            # データ変換
            dom = parse(data)
            index = list(map(lambda x: x['areajp'], dom['radiru_config']['stream_url']['data'])).index(self.areajp)
            ...
            # 設定データ
            buf = []
            template = '    <setting label="{name}" type="bool" id="radiru_{id}" default="true" enable="eq({offset},2)"/>'
            for i, s in enumerate(self.STATION):
                buf.append(template.format(id=s['id'], name=s['name'], offset=-1 - i))
            # 設定データを書き込む
    ...
    def getProgramData(self, renew=False):
        ...
        if renew or not os.path.isfile(self.PROGRAM_FILE) or timestamp() > read_file(self.NEXTUPDT_FILE):
            ...
            try:
                ...
                data = urlread(url)
                write_json(self.PROGRAM_FILE, json.loads(data))
            except Exception:
